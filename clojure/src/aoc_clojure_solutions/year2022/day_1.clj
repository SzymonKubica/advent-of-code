...
(defn get-vectors-splitting-on-empty-lines-explicit-recur
  "Given a list of string lines, each line being either blank or containing
  a single integer, it returns a vector of vectors of integers, delimited on blank lines.
  ...
  (if (empty? lines)
    ...
    (if (str/blank? (first lines))
      ; Blank line: need to end the current vector and add it to the list.
      (get-vectors-splitting-on-empty-lines-explicit-recur
        (tail lines) (conj vectors curr) [])
      ; Line with a number: parse the number and append it to the current vector
      (get-vectors-splitting-on-empty-lines-explicit-recur
        (tail lines) vectors (conj curr (Integer/parseInt (first lines)))))))


(defn vectors-grouped-at-empty-lines
  "Given a list of string lines, each line being either blank or containing
  a single integer, it returns a vector of vectors of integers, delimited on
  blank lines."
  [lines]
  (->> lines
...
(defn first_part [file-name]
  (let [file-lines (read-file-lines file-name)
        food-carried-by-each-elf (vectors-grouped-at-empty-lines file-lines)
